...
# Función para extraer puertos del docker-compose.yml
extraer_puertos() {
...
# Liberar puertos explícitos del docker-compose.yml
...
# Resumen final de todos los puertos
...
for PUERTO in $ALL_PORTS; do
  ...
  if [[ "$OSTYPE" == "msys" || "$OSTYPE" == "win32" ]]; then
    ...
    STATUS=$(lsof -ti tcp:$PUERTO_NUM >/dev/null && echo -e "${RED}OCUPADO${NC}" || echo -e "${GREEN}LIBRE${NC}")
  fi
